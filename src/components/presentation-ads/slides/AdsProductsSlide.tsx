import { Package, TrendingUp, TrendingDown, Sparkles } from "lucide-react";
import { produtoAdsOutubro, produtoAdsNovembro, totalAdsOutubro, totalAdsNovembro } from "@/data/dashboardAdsData";

export function AdsProductsSlide() {
  return (
    <div className="flex flex-col h-full px-12 py-8">
      <div className="text-center mb-8 animate-fade-in">
        <div className="flex items-center justify-center gap-3 mb-2">
          <Package className="w-8 h-8 text-primary" />
          <h2 className="text-4xl font-bold text-foreground">Análise por Produto</h2>
        </div>
        <p className="text-xl text-muted-foreground">Distribuição de anúncios por produto</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-8 flex-1 animate-fade-in" style={{ animationDelay: "200ms" }}>
        ...
        <div className="p-6 rounded-xl bg-blue-500/5 border border-blue-500/20">
          ...
          <div className="space-y-3">
            {produtoAdsOutubro.filter(p => p.name !== "VAZIO").map((item, index) => (
        ...
        <div className="p-6 rounded-xl bg-purple-500/5 border border-purple-500/20">
          ...
          <div className="space-y-3">
            {produtoAdsNovembro.map((item, index) => (
              <div key={item.name} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                <div className="flex items-center gap-3">
                  <span className="text-muted-foreground text-sm w-5">{index + 1}.</span>
                  <span className="text-foreground font-medium">{item.name}</span>
                  {!produtoAdsOutubro.find(p => p.name === item.name) && (
      ...
      <div className="grid grid-cols-3 gap-4 mt-6 animate-fade-in" style={{ animationDelay: "400ms" }}>
        <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/20 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">KARYLIEF: 1 → 17 (+1600%)</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">LAELLIUM: 17 → 0 (-100%)</p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-blue-400">Novos Produtos</p>
            <p className="text-sm text-muted-foreground">JERTARIS, KORVIZOL</p>
          </div>
        </div>
      </div>
    </div>
